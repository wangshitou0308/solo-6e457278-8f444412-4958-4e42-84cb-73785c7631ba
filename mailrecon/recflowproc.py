...
与作业/案件/时序/身份/附件处理器结构一致：单后台线程顺序消费分析队列，
逐个加载目标（作业或案件）的结果 JSON，扁平化会话森林（恢复父子关系、
会话根与来源描述，保留 From/To/Cc 可见地址与 identity 头块）后交给
recipientflow 引擎分析，结果独立落盘到
``DATA_DIR/recipient_flows/<flow_id>/``——删除源作业/案件不影响已完成
分析的读取；服务重启时未完成的分析自动重新入队。
"""
...
from . import config, jsonio, recipientflow
from .storage import Storage, utc_now


class RecipientFlowProcessor:
    """单后台线程顺序处理收件人流转分析队列。"""
    ...
    def __init__(self, storage: Storage) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="recipient-flow-processor", daemon=True
        )

    def start(self) -> None:
        # 重启恢复：上次未完成的分析重新入队
        for flow in self._storage.unfinished_recipient_flows():
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                ...
                self._storage.fail_recipient_flow(
                    flow_id,
    ...
    def _process(self, flow_id: str) -> None:
        flow = self._storage.get_recipient_flow(flow_id)
        if flow is None:
            return
        target_type = flow["target_type"]
        target_id = flow["target_id"]

        self._storage.update_recipient_flow_progress(
        ...
        # ---- 3. 收件人流转分析 ------------------------------------------
        self._storage.update_recipient_flow_progress(
            flow_id, progress=60, phase="analyzing_recipients"
        )
        outcome = recipientflow.analyze(emails)
        ...
        flow_dir = Storage.recipient_flow_dir(flow_id)
        ...
        self._storage.complete_recipient_flow(
            flow_id,
            str(result_path),
...
def _flatten(
    ...
    """迭代式摊平会话森林，恢复 parent_uid、会话根、可见地址与来源描述。"""
